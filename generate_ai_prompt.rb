...
def run_cycle_for(symbol)
  ...
  if nearest_expiry
    ...
    if oc.is_a?(Hash)
      ...
      pcr = call_oi.positive? ? (put_oi.to_f / call_oi) : 0.0
    end
  end
  ...
  ai_prompt = <<~PROMPT
    Analyze the following live market data for #{symbol} options trading (weekly expiry: #{nearest_expiry || 'N/A'}) and suggest whether to go long on CE (Call Option) for a bullish reversal or long on PE (Put Option) for a bearish reversal. Provide reasoning based on PCR, technical indicators, and price action. Aim for intraday trades with 20k capital, targeting 5k profit per day as per the PCR Trend Reversal Strategy.

    - Current Spot Price: #{format_num(spot_price)}
    - Current OHLC: #{current_ohlc_str}
    - Total Call OI: #{call_oi}
    - Total Put OI: #{put_oi}
    - PCR (Put/Call Ratio): #{format_num(pcr)} (Rising PCR suggests bullish sentiment, falling suggests bearish)
    - 20-Period SMA: #{format_num(sma_20)}
    - 14-Period RSI: #{format_num(rsi_14)} (Overbought >70, Oversold <30)
    - Recent Price Change (last 5-min): #{last_change}%
